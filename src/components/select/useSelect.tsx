import {
  ...
  SelectFormValue,
  IOptionContents,
} from '@src/store/SelectState';
import { useEffect, useState } from 'react';

import { useRecoilValue, useSetRecoilState } from 'recoil';
import { ISelectOptionData } from './types';

export function useSelect(id: string, selectType: 'single' | 'multiple') {
  const setSelectState = useSetRecoilState(SelectState);
  const usedSelectState = useRecoilValue(getSelectState(id)); //현재 select 값
  const selectFormValue = useRecoilValue(SelectFormValue(id)); //현재 select에서 선택된 값
  const optionDataInit: ISelectOptionData = {
    label: '',
    layerSplit: '1',
    selectId: [],
    contents: [],
